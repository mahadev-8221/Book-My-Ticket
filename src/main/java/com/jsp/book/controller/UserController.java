...
public class UserController {
	...
	@GetMapping("/search")
	public String searchMovies(@RequestParam String query, ModelMap model) {
		return userService.searchMovies(query, model);
	}

	@GetMapping("/login")
	public String loadLogin(LoginDto loginDto) {
		return "login.html";
	}

	@PostMapping("/login")
	public String login(@Valid LoginDto dto, BindingResult result, RedirectAttributes attributes, HttpSession session) {
		return userService.login(dto, result, attributes, session);
	}
	...
	public String forgotPassword(ForgotPasswordDto forgotPasswordDto) {
		return userService.loadForgotPassword(forgotPasswordDto);
	}

	@PostMapping("/forgot-password")
	public String forgotPassword(@Valid ForgotPasswordDto forgotPasswordDto, BindingResult result,
			RedirectAttributes attributes) {
		return userService.forgotPassword(forgotPasswordDto, result, attributes);
	}
	...
	public String saveSeats(@PathVariable Long id, @Valid SeatLayoutForm seatLayoutForm, BindingResult result,
			HttpSession session, RedirectAttributes attributes) {
		return userService.saveSeats(id, seatLayoutForm, result, session, attributes);
	}
	...
	public String confirmTicket(HttpSession session, ModelMap map, RedirectAttributes attributes,
			...
			@RequestParam(required = false) String razorpay_signature) throws IOException, WriterException {
		if (razorpay_signature != null) {
			session.setAttribute("razorpay_signature", razorpay_signature);
		}
		return userService.confirmTicket(session, map, attributes, razorpay_order_id, razorpay_payment_id);
	}

	@GetMapping("/bookings")
	public String viewBookings(HttpSession session, ModelMap model, RedirectAttributes attributes) {
		return userService.viewBookings(session, model, attributes);
	}

	@GetMapping("/user-bookings/{id}")
	public String viewUserBookings(@PathVariable Long id, HttpSession session, ModelMap map,
			RedirectAttributes attributes) {
		return userService.viewUserBookings(id, session, map, attributes);
	}
	...
	public String manageAllBookings(HttpSession session, ModelMap map, RedirectAttributes attributes) {
		return userService.manageAllBookings(session, map, attributes);
	}

	@GetMapping("/approve-booking/{id}")
	public String approveBooking(@PathVariable Long id, HttpSession session, RedirectAttributes attributes) {
		return userService.approveBooking(id, session, attributes);
	}

	@GetMapping("/reject-booking/{id}")
	public String rejectBooking(@PathVariable Long id, HttpSession session, RedirectAttributes attributes) {
		return userService.rejectBooking(id, session, attributes);
	}
}
